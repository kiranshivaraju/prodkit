...
### Step 6: Validate Sprint Review Created

Run validation checks to ensure sprint review was created successfully:

Display validation in progress:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  VALIDATING SPRINT REVIEW
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Check 1: Sprint Review File Exists**
```bash
SPRINT_NUM=$(grep "current:" .prodkit/config.yml | sed 's/.*current: //')
REVIEW_FILE="sprints/v${SPRINT_NUM}/sprint-review.md"

if [ ! -f "$REVIEW_FILE" ]; then
    echo "❌ Validation failed: Sprint review not found"
    echo "Expected: $REVIEW_FILE"
    exit 1
fi

echo "✓ Sprint review file exists"
```

**Check 2: Sprint Review Has Content**
```bash
if [ ! -s "$REVIEW_FILE" ]; then
    echo "❌ Validation failed: Sprint review is empty"
    exit 1
fi

echo "✓ Sprint review has content"
```

**Check 3: Metrics Section Exists**
```bash
if ! grep -q "## Metrics" "$REVIEW_FILE"; then
    echo "❌ Validation failed: Metrics section missing"
    echo "Sprint review must include metrics data"
    exit 1
fi

echo "✓ Metrics section exists"
```

**Check 4: Code Walkthrough Section Exists**
```bash
if ! grep -q "## Code Walkthrough" "$REVIEW_FILE"; then
    echo "⚠️  Warning: Code walkthrough section missing"
    echo "Consider adding detailed code walkthrough for documentation"
else
    echo "✓ Code walkthrough section exists"
fi
```

**Check 5: Sprint Had Activity**
```bash
# Check if sprint had closed issues
CLOSED_ISSUES_COUNT=$(grep -c "^- #[0-9]" "$REVIEW_FILE" | grep -A1 "## Closed Issues" || echo "0")

if [ -z "$CLOSED_ISSUES_COUNT" ] || [ "$CLOSED_ISSUES_COUNT" -eq 0 ]; then
    echo "⚠️  Warning: No closed issues found in sprint review"
else
    echo "✓ Sprint review includes closed issues"
fi

# Check if sprint had merged PRs
if grep -q "## Merged Pull Requests" "$REVIEW_FILE"; then
    echo "✓ Sprint review includes merged PRs"
else
    echo "⚠️  Warning: No merged PRs section found"
fi
```

**Check 6: Sprint Plan Exists (prerequisite)**
```bash
SPRINT_PLAN="sprints/v${SPRINT_NUM}/sprint-plan.md"

if [ ! -f "$SPRINT_PLAN" ]; then
    echo "⚠️  Warning: Sprint plan not found"
    echo "Run /prodkit.plan-sprint first to create a sprint plan"
else
...
✓ Sprint review validated with all required sections
